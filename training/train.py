import numpy as np
import logging
from utils import save_agent, save_results, record_agent
...
def train_agent(agent, env, game_name: str, num_episodes: int, new_agent, episode_score: list, num_frames: int, save_freq: int, record_freq: int, update_freq_target: int, update_freq_policy: int = 4):
    ...
    for i_episode in range(num_episodes):
        ...
        if (i_episode + 1) % record_freq == 0:
            ...
            save_results(game_name, episode_score, starting_episode + i_episode + 1)
            record_agent(agent, env, game_name, starting_episode + i_episode + 1)